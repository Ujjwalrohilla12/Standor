    ...
    <AppFrame label="Standor — Dashboard">
      <div className="space-y-2">
        ...
        <div className="bg-white/[0.02] rounded-lg p-3 border border-white/[0.05]">
          ...
          >
            <polyline
              points="0,35 20,28 40,30 60,15 80,20 100,10 120,18 140,12 160,22 180,8 200,14"
              fill="none"
            ...
            />
            <polyline
              points="0,40 0,35 20,28 40,30 60,15 80,20 100,10 120,18 140,12 160,22 180,8 200,14 200,40"
        ...
        <div className="space-y-1">
          {[
            "capture-2026-02-28.pcap",
            "incident-001.pcapng",
            "lateral-move.pcap",
          ].map((f, i) => (
            <div
              key={f}
              className="flex items-center justify-between bg-white/[0.02] rounded-md px-3 py-2 border border-white/[0.04]"
            >
              <span className="text-[9px] font-mono text-neutral-400 truncate">
                {f}
              </span>
              <span
                className={`text-[8px] font-bold uppercase px-1.5 py-0.5 rounded ${i === 1 ? "text-red-400 bg-red-500/10" : "text-green-400 bg-green-500/10"}`}
              >
                {i === 1 ? "Alert" : "Clean"}
              </span>
            </div>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-4 gap-1">
          {[
            ...
            <div
              key={p}
              className="bg-white/[0.02] rounded-md p-2 border border-white/[0.04] text-center"
            >
              <div
        ...
        <div className="space-y-0.5">
          {[
            ...
            <div
              key={n}
              className="grid grid-cols-6 gap-1 text-[8px] font-mono bg-white/[0.015] rounded px-2 py-1.5"
            >
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          {[
            ...
            <div
              key={k}
              className="flex items-center justify-between px-2 py-1 bg-white/[0.02] rounded border border-white/[0.04]"
            >
              <span className="text-[9px] text-neutral-500">{k}</span>
              <span className={`text-[9px] font-mono font-bold ${vc}`}>
                {v}
              </span>
            </div>
          ))}
        ...
        <div className="bg-white/[0.02] rounded-lg p-2 border border-white/[0.05]">
          <div className="flex gap-1 mb-2">
            {["HEX", "BASE64", "ASCII", "URL"].map((m, i) => (
              <span
                key={m}
                className={`text-[8px] font-bold px-2 py-0.5 rounded ${i === 0 ? "bg-white/10 text-white" : "text-neutral-600"}`}
              >
                {m}
              </span>
            ))}
          ...
          <div className="font-mono text-[8px] text-green-400/70 leading-relaxed">
            47 45 54 20 2f 61 70 69 2f 73 65 73 73 69 6f 6e
            <br />
        ...
        <div className="flex items-center gap-2 px-2 py-1.5 bg-white/[0.02] rounded-lg border border-white/[0.04]">
          ...
          <div className="flex -space-x-1.5">
            {["A", "B", "C"].map((l, i) => (
              <div
                key={l}
                className="w-5 h-5 rounded-full border border-black flex items-center justify-center text-[7px] font-bold"
          ...
          {[
            {
              user: "Alice",
              color: "#3b82f6",
              msg: "Suspicious DNS query to 185.x.x.x — possible C2 beacon",
              time: "14:02",
            },
            {
              user: "Bob",
              color: "#22c55e",
              msg: "Confirmed — entropy spike on port 443 payload matches exfil pattern",
              time: "14:04",
            },
            {
              user: "Carol",
              color: "#f59e0b",
              msg: "Escalating to IR — tagging packet #847 as IOC",
              time: "14:05",
            },
    ...
    <AppFrame label="Admin Console — Security & Audit">
      <div className="space-y-2">
          ...
          {[
            ...
            >
              ...
              <div className="flex items-end gap-2">
                ...
                <span
                  className={`text-[8px] font-mono ${d.startsWith("+") ? "text-green-400" : "text-red-400"}`}
                >
                  {d}
                </span>
              </div>
        ...
        <div className="bg-white/[0.02] rounded-lg border border-white/[0.05]">
          <p className="text-[8px] text-neutral-500 px-2 pt-2 pb-1.5 border-b border-white/[0.04]">
            Recent Audit Events
          </p>
          <div className="divide-y divide-white/[0.04]">
            {[
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${status === "success" ? "bg-green-400" : "bg-red-400"}`}
                />
                <span className="text-[8px] text-neutral-400 flex-1 truncate">
                  {action}
                </span>
                <span className="text-[8px] font-mono text-neutral-600 truncate max-w-[80px]">
                  {user}
                </span>
  ...
  return (
    <AppFrame label="Entropy Radar — Anomaly Detection">
      <div className="space-y-2">
        <div className="bg-white/[0.02] rounded-lg p-3 border border-white/[0.05]">
          <p className="text-[9px] text-neutral-500 mb-2">
            Shannon entropy by flow — last 500 packets
          </p>
          {/* Bar chart */}
          <div className="flex items-end gap-1 h-16">
            {[
              0.3, 0.4, 0.35, 0.55, 0.45, 0.38, 0.92, 0.88, 0.91, 0.42, 0.36,
              0.5, 0.48, 0.37, 0.44,
            ].map((v, i) => (
              <div
                key={i}
...
const ITEMS: GalleryItem[] = [
  {
    ...
    description:
      "Centralised overview of active sessions, ingest metrics, alert counts, and recent captures at a glance.",
  ...
  },
  {
    ...
    badge: "New",
    description:
      "Server-side Shannon entropy calculations surface high-entropy flows instantly — flag encrypted tunnels, exfiltration, or anomalous payloads without opening a single packet.",
  ...
  },
  {
    ...
    description:
      "Scrub through millions of packets with sub-millisecond precision. Jump to any timestamp in O(log n) time using the indexed time-warp engine.",
  ...
  },
  {
    ...
    description:
      "L2–L7 field breakdown with live payload transforms: HEX, BASE64, ASCII, and URL decode. Read-only sandboxed execution — no native code, no risk.",
    visual: <PacketMockup />,
  },
  {
    id: "collaboration",
    ...
    description:
      "Multiple investigators annotate, tag, and escalate packets simultaneously. Yjs CRDTs guarantee convergent state across all sessions — even with network partitions.",
  ...
  },
  {
    ...
    description:
      "Append-only audit trail for every auth event, API key operation, and privilege change. Full admin visibility with per-user drill-down and tamper-evident logs.",
    visual: <AdminMockup />,
  },
];
    ...
    <main className="min-h-screen bg-ns-bg-900 text-white">
      ...
      <section className="pt-32 pb-16 px-6">
        <div className="max-w-5xl mx-auto text-center">
          <p className="text-[11px] font-bold tracking-[0.25em] uppercase text-neutral-500 mb-6">
            Product Gallery
          </p>
          <h1 className="text-4xl sm:text-5xl font-black tracking-tight mb-5">
            Built for how
            <br className="hidden sm:block" /> investigators actually work
          </h1>
          <p className="text-base text-neutral-400 max-w-2xl mx-auto mb-10 leading-relaxed">
            A visual tour of Standor's investigation surface — from
            high-throughput ingest to real-time collaborative forensics.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filtered.map((item) => (
              ...
              >
                ...
                >
                  <div
                    className={`transition-all duration-500 ${expanded === item.id ? "scale-100" : "scale-[0.97] group-hover:scale-100"}`}
                  >
                    {item.visual}
                  </div>
                </div>
