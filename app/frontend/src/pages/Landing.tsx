...
export default function Landing() {
  ...
  return (
    <main className="w-full min-h-screen bg-[linear-gradient(180deg,#050608_0%,#07090b_48%,#090b0e_100%)] text-white font-sans overflow-x-hidden selection:bg-white/30 text-[15px]">

      {/* ── HERO SECTION ── */}
      <section className="relative w-full min-h-[90vh] flex flex-col justify-center pt-24 pb-16 px-6 lg:px-12 bg-[linear-gradient(180deg,#050608_0%,#07090b_100%)]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-lg sm:text-xl text-[#3b3f44] font-semibold mb-10 max-w-lg leading-relaxed">
            ...
            <div className="flex flex-wrap items-center gap-4">
              <Link to="/join" className="px-10 py-3.5 rounded-full border border-white bg-white text-black font-semibold hover:bg-zinc-100 transition-all flex items-center gap-2 shadow-[0_10px_28px_rgba(255,255,255,0.08)] hover:shadow-[0_16px_36px_rgba(196,199,206,0.14)]">
                Join Meeting
                <Play size={16} fill="currentColor" />
              </Link>
              <Link to="/login" target="_blank" rel="noopener noreferrer" className="px-10 py-3.5 rounded-full border border-white/20 bg-black text-white font-medium hover:bg-white/5 transition-all shadow-[0_8px_22px_rgba(0,0,0,0.2)] hover:shadow-[0_14px_30px_rgba(196,199,206,0.08)]">
                Login
              </Link>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.2 }}
            ...
            <div className="relative w-full h-full p-1 rounded-[28px] bg-zinc-800/20 shadow-2xl">
              <div ref={demoRef} className="w-full h-full rounded-[24px] overflow-hidden flex flex-col" style={{ backgroundColor: EDITOR_THEME.frame, border: `1px solid ${EDITOR_THEME.border}` }}>
                {/* Editor Top Bar Mock */}
                <div className="h-10 px-4 flex items-center gap-3" style={{ backgroundColor: EDITOR_THEME.topBar, borderBottom: `1px solid ${EDITOR_THEME.border}` }}>
                  <div className="flex gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-full bg-zinc-700" />
                    <div className="w-2.5 h-2.5 rounded-full bg-zinc-600" />
                    <div className="w-2.5 h-2.5 rounded-full bg-zinc-500" />
                  </div>
                  <span className="text-xs font-mono ml-2" style={{ color: EDITOR_THEME.muted }}>twoSum.ts</span>
                ...
                {/* Editor Content Mock */}
                <div className="flex-1 p-5 font-mono text-xs sm:text-sm leading-6 sm:leading-7 overflow-hidden" style={{ color: EDITOR_THEME.text }}>
                  {CODE_LINES.map((line, i) => (
                    <div key={line.key} className={`flex items-center transition-all duration-300 ${i < visibleLines ? 'opacity-100' : 'opacity-0'} ${(line as any).hl ? 'text-white px-1 rounded -mx-1' : ''}`} style={(line as any).hl ? { backgroundColor: EDITOR_THEME.highlightBg } : undefined}>
                      <span className="w-6 text-right mr-4 select-none" style={{ color: EDITOR_THEME.lineNumber }}>{i + 1}</span>
                      <span>{line.code}</span>
                ...
                <div className="h-10 px-4 flex items-center justify-between" style={{ backgroundColor: EDITOR_THEME.bottomBar, borderTop: `1px solid ${EDITOR_THEME.border}` }}>
                  <span className="text-[10px] font-mono" style={{ color: '#6e7681' }}>TypeScript · WebRTC Sync</span>
                  <div className="px-3 py-1 rounded text-[10px] font-bold border" style={{ backgroundColor: EDITOR_THEME.pillBg, color: EDITOR_THEME.pillText, borderColor: 'rgba(255,255,255,0.05)' }}>● Connected</div>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      {/* ── ABOUT SECTION ── */}
      <section id="about" className="w-full py-24 px-6 lg:px-12 bg-[linear-gradient(180deg,#07090b_0%,#080a0d_100%)] border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-bold text-center mb-16 text-white uppercase tracking-widest">About Standor</h2>
          ...
          <div className="grid lg:grid-cols-2 gap-16 lg:gap-12 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <p className="text-[#4b4f55] text-lg leading-relaxed font-semibold">
      ...
      {/* ── FEATURES SECTION ── */}
      <section id="features" className="w-full py-24 px-6 lg:px-12 bg-[linear-gradient(180deg,#080a0d_0%,#090b0e_100%)] border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl sm:text-4xl font-bold text-center mb-16 text-white uppercase tracking-widest">Features</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6 lg:gap-8">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-black border border-zinc-800 rounded-2xl overflow-hidden group hover:border-white transition-all duration-300">
              <div className="h-48 sm:h-56 p-6 flex justify-center items-center bg-zinc-950/90">
                <div className="w-full max-w-sm h-full bg-[#161b22] border border-white/5 rounded-[1.25rem] shadow-[0_18px_45px_rgba(0,0,0,0.28)] relative overflow-hidden flex flex-col">
                  <div className="h-8 border-b border-white/5 bg-[#0d1117] px-3 flex items-center">
                    <span className="text-[10px] text-[#2f3237] font-bold uppercase tracking-wider">Rooms</span>
                  </div>
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-[#3b3f44] font-semibold">Create and manage your own collaborative coding rooms instantly.</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="bg-black border border-zinc-800 rounded-2xl overflow-hidden group hover:border-white transition-all duration-300">
              <div className="h-48 sm:h-56 p-6 flex justify-center items-center bg-zinc-950/90">
                <div className="w-full max-w-sm h-full flex gap-3">
                  <div className="flex-1 bg-[#161b22] border border-white/5 rounded-[1.25rem] flex flex-col p-3 shadow-[0_18px_45px_rgba(0,0,0,0.28)]">
                    ...
                    <div className="mt-auto h-24 bg-zinc-900 rounded-[1rem] flex items-end justify-center pb-2">
                      <Brain className="text-[#4b4f55] opacity-90" size={32} />
                    </div>
                  </div>
                </div>
              </div>
              <div className="p-6">
                ...
                <p className="text-sm text-[#3b3f44] font-semibold">Keep a personalized, automated journal entry for every session you host.</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} className="bg-black border border-zinc-800 rounded-2xl overflow-hidden group hover:border-white transition-all duration-300">
              <div className="h-48 sm:h-56 p-6 flex justify-center items-center bg-zinc-950/90">
                <div className="w-full max-w-sm h-full bg-[#161b22] border border-white/5 rounded-[1.25rem] shadow-[0_18px_45px_rgba(0,0,0,0.28)] relative p-4 flex gap-4">
                  <div className="w-16 h-[120%] -mt-6 rounded-[1rem] overflow-hidden shadow-2xl bg-zinc-800 flex-shrink-0 border border-white/10 opacity-60"></div>
                  <div className="w-16 h-[120%] -mt-6 rounded-[1rem] overflow-hidden shadow-2xl bg-zinc-600 flex-shrink-0 border border-white/10 opacity-70"></div>
                  <div className="w-16 h-[120%] -mt-6 rounded-[1rem] overflow-hidden shadow-2xl bg-zinc-700 flex-shrink-0 border border-white/10 opacity-80"></div>
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-lg font-bold text-white mb-2">Top Library Problems</h3>
                <p className="text-sm text-[#3b3f44] font-semibold">Effortlessly curate and showcase your top coding challenges in a sleek, customizable list.</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.3 }} className="bg-black border border-zinc-800 rounded-2xl overflow-hidden group hover:border-white transition-all duration-300">
              <div className="h-48 sm:h-56 p-6 flex justify-center items-center bg-zinc-950/90">
                <div className="w-full max-w-sm h-full bg-[#161b22] border border-white/5 rounded-[1.25rem] shadow-[0_18px_45px_rgba(0,0,0,0.28)] relative flex flex-col overflow-hidden">
                  <div className="p-3 border-b border-white/5 flex gap-2">
                    <span className="text-[10px] bg-white/10 text-white px-2 py-0.5 rounded-full border border-white/10">Python</span>
                    <span className="text-[10px] bg-white/5 text-[#3b3f44] px-2 py-0.5 rounded-full border border-white/5">JavaScript</span>
                    <span className="text-[10px] bg-white text-black px-2 py-0.5 rounded-full font-bold">Rust</span>
                  </div>
              ...
              <div className="p-6">
                ...
                <p className="text-sm text-[#3b3f44] font-semibold">Get lightning-fast, secure execution for your code across 12+ real runtime environments.</p>
              </div>
            </motion.div>
